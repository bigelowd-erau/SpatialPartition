...
namespace SpatialPartitionPattern
{
    public class GameController : MonoBehaviour
    {
        ...
        public Toggle useSpacialPartitionToggle;
        private float updateLoopStartTime;
        ...
        void Update()
        {
            updateLoopStartTime = Time.realtimeSinceStartup;
            //Move the enemies
            ...
            for (int i = 0; i < friendlySoldiers.Count; i++)
            {
                ...
                if (useSpacialPartitionToggle.isOn)
            ...
            Debug.Log(((Time.realtimeSinceStartup - updateLoopStartTime) * 1000f) + "ms");
        }

        public void Reload(int newNumSoldiers)
        {
            numberOfSoldiers = newNumSoldiers;
            foreach (Enemy enemy in enemySoldiers)
                Destroy(enemy.thisSoldier);
            enemySoldiers.Clear();

            foreach (Friendly friend in friendlySoldiers)
                Destroy(friend.thisSoldier);
